...
export default function ProductDetailsClient({ initialProduct, slug }: ProductDetailsClientProps) {
  ...
  const handleDirectBuy = async () => {
    ...
    try {
      // Add item to cart using the normal cart flow
      addToCart(product, quantity, selectedSize, selectedColor);
      
      console.log('🛒 [Direct Buy] Item added to cart, redirecting to checkout');
  ...
  return (
    <>
      ...
      <div className="bg-white min-h-screen">
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="hidden md:block">
              {/* Main Image */}
              <div className="relative w-full aspect-3/4 bg-gray-100 overflow-hidden mb-4 rounded-lg">
                <Image
              ...
              <div className="grid grid-cols-4 gap-3">
                {images.map((img: string, idx: number) => (
                  <button
                    ...
                    className={`relative w-full aspect-3/4 bg-gray-100 overflow-hidden rounded cursor-pointer transition-all ${
            ...
            <div className="flex flex-col">
              ...
              <div className="mb-6">
                ...
                {/* Price with DiscountBadge */}
                <div className="flex items-center gap-4 mb-4 flex-wrap">
                  {product?.originalPrice && product?.originalPrice !== product?.price && (
                    <p className="text-lg line-through text-red-500">₹{product.originalPrice}</p>
                  )}
              ...
              {/* Action Buttons - Desktop Sticky Section */}
              <div className={`hidden md:block mt-8 transition-all duration-300 ${hasScrolled ? 'sticky top-0 z-40 bg-white shadow-md py-3 -mx-4 px-4' : ''}`}>
        ...
        {/* Mobile: Fixed Bottom CTA - Only show when scrolled and near footer */}
        {hasScrolled && (
          <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 safe-area-inset-bottom z-50 animate-slide-in-up">
            <div className="flex flex-col gap-2">
              ...
              {/* Row 2: Buy Button (Direct Checkout) */}
              <button
                onClick={handleDirectBuy}
                disabled={isOutOfStock || !selectedSize || !selectedColor || isDirectBuying}
              ...
              >
                {isOutOfStock ? 'Out of Stock' : isDirectBuying ? 'Processing...' : 'BUY'}
              </button>
            </div>
          </div>
        )}
