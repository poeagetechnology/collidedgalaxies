...
function ProductsPageContent() {
  ...
  if (selectedFilters.categories.length > 0) {
    filteredProducts = filteredProducts.filter(p => {
      ...
      // Handle "Plain" category - show only plain products
      if (selectedCategory.toLowerCase() === 'plain') {
        const isPlain = (p.title || '').toLowerCase().includes('plain') || 
                       (p.slug || '').toLowerCase().includes('plain');
        return isPlain;
      }
      
      // Handle "Oversized" category - show only oversized (non-plain) products
      if (selectedCategory.toLowerCase() === 'oversized') {
        const isPlain = (p.title || '').toLowerCase().includes('plain') || 
                       (p.slug || '').toLowerCase().includes('plain');
        return !isPlain; // Show non-plain products only
      }
      
      // For other categories, check the category field
      return selectedFilters.categories.includes(p.category || '');
    });
  }
  ...
  if (selectedSort === "A-Z") {
    ...
    // Default sort: Show non-plain t-shirts first, then plain t-shirts
    filteredProducts = [...filteredProducts].sort((a, b) => {
      ...
      // If one is plain and the other isn't, plain comes after
      if (aIsPlain && !bIsPlain) return 1;
      if (!aIsPlain && bIsPlain) return -1;
      
      // If both are same type (both plain or both non-plain), maintain order by createdAt
  ...
  return (
    <>
      ...
      <section className="bg-white min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-12 md:pt-30 pb-10">
              ...
              return (
                ...
                >
                  <Link href={productUrl} className="group">
                    <motion.div
                      className={`aspect-3/4 relative w-full overflow-hidden bg-gray-100 rounded-lg
