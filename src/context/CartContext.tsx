...
export type CartItem = {
  ...
  isFlexibleBundle?: boolean; // ✅ Mark for flexible bundles like "Pick Any 3"
...
export default function CartProvider({
  ...
  const generateUniqueKey = (
    ...
    const keyString = `${productId || title}-${size}-${normalizedColor}`;
    let hash = 0;
          ...
          const unsubscribe = onSnapshot(
                    ...
                    try {
                      ...
                      try {
                        ...
                        if (firestoreError.code === "permission-denied") {
                          console.warn(
                            "Firestore permissions denied - keeping cart in localStorage",
                          );
            ...
            (error: any) => {
              ...
              if (error.code === "permission-denied") {
                console.warn(
                  "Firestore permissions denied - using localStorage only",
                );
  ...
  const saveCart = async (items: CartItem[]) => {
    try {
      // Clean items to remove undefined values (Firestore doesn't allow undefined)
      ...
      if (user) {
        ...
        try {
          ...
          if (firestoreError.code === "permission-denied") {
            console.warn(
              "Firestore permissions denied - saving to localStorage instead",
            );
            localStorage.setItem(
              CART_STORAGE_KEY,
        ...
        localStorage.setItem(CART_STORAGE_KEY, JSON.stringify(cleanedItems));
      }
    } catch (error) {
  ...
  const clearCart = async () => {
    ...
    if (user) {
      try {
        ...
        if (error.code === "permission-denied") {
          console.warn(
            "Firestore permissions denied - cart cleared locally only",
          );
        }
      }
    }
