...
export async function generateShiprocketAccessToken(
  ...
  try {
    ...
    console.log('🚀 [Client] Generating Shiprocket token with payload:', payload);
    ...
    console.log('📨 [Client] Token API response status:', response.status);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('❌ [Client] API Error (text):', text);
    ...
    console.log('✅ [Client] Received token:', data.token?.substring(0, 10) + '...');
