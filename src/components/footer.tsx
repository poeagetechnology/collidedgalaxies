...
import { getFooterContactInfo } from "@/src/server/services/footerContact.service";
...
export default function Footer() {
  ...
  useEffect(() => {
    const fetchContactInfo = async () => {
      try {
        const info = await getFooterContactInfo();
        setContactInfo(info);
      } catch (error) {
    ...
    <footer className="relative bg-black text-white pt-8 sm:pt-10 md:pt-12 pb-8 sm:pb-10 md:pb-12 z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:w-2/3 text-left">
          ...
          <div>
            ...
            <ul className="space-y-2 text-md text-gray-300 leading-relaxed">
              ...
              {contactInfo.phoneNumbers && contactInfo.phoneNumbers.map((phone, index) => (
                <li key={index}>
                  <Link href={`tel:${phone.replace(/\s/g, '')}`} className="hover:text-white transition">{phone}</Link>
                </li>
              ))}
              <li>
                <Link href={`mailto:${contactInfo.email}`} className="hover:text-white transition">{contactInfo.email}</Link>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-sm text-gray-500 text-center">
        ...
        <div className="space-y-2">
          <div>
            © 2026 Collided Galaxies. All rights reserved.
          </div>
          <div>
            Crafted By <Link href="https://www.poeagetechnology.com" target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:text-blue-200 transition font-semibold">Poeage</Link>
          </div>
        </div>
      </div>
    </footer>
