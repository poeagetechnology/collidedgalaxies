...
const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const handleAddImage = async () => {
    // No limit on image count anymore
    if (imageFiles.length === 0 && !imageInput) {
  ...
  const handleSave = async () => {
    ...
    // If there are pending image previews, they need to be added first
    if (imagePreviews.length > 0 && imageFiles.length > 0) {
      alert("Please click 'Add Image(s)' button to add the selected images first!");
      return;
    }
