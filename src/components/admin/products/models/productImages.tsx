...
const ProductImages: React.FC<Props> = ({
  ...
  return (
    <div className="border p-3 sm:p-4 flex flex-col gap-3">
      <h2 className="font-semibold text-sm sm:text-base">Product Images</h2>
      ...
      <button
        ...
        disabled={isUploading || (imageFiles.length === 0 && !imageInput)}
        className={`w-full px-4 py-2 text-white text-sm font-semibold ${
          isUploading || (imageFiles.length === 0 && !imageInput)
      ...
      >
        {isUploading
          ...
          : imagePreviews.length > 0
          ? `Add Image(s) - Click to Add ${imagePreviews.length} selected file(s)`
          : `Add Image(s)`}
      </button>
    </div>
  );
